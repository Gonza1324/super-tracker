...
import { UNIT_VALUES, type Unit } from '@/features/products/units'
...
export function defaultPurchaseItem(): PurchaseFormValues['items'][0] {
  return {
    product_id: '',
    product: null,
    quantity: 1,
...
export function emptyPurchaseDefaults(): PurchaseFormValues {
  return {
    store: null,
    store_id: null,
              ...
              <div>
                ...
                <ProductCombobox
                  ...
                  onChange={(p) => {
                    ...
                    if (p?.default_unit) {
                      setValue(`items.${index}.unit`, p.default_unit as Unit)
                    }
                  }}
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select value={field.value} onValueChange={field.onChange}>
                ...
                <SelectContent>
                  {UNIT_VALUES.map(u => <SelectItem key={u} value={u}>{u}</SelectItem>)}
                </SelectContent>
              </Select>
            )}
          />
...
export function PurchaseForm({ groupId, defaultValues, submitLabel, isPending, onSubmit }: Props) {
  const {
    register,
    control,
    handleSubmit,
